...
This study is to answer the above problems. Target market is Hong Kong Stock Exchange Market. Objects is to find a good method to predict HSI stock price changes through testing on different machine learning algorithms (includes Linear Regression, Neural Network, Random Forest). Apart from using single learning methods, this study also tries to combine two machine learning methods to do that predict, which use one to forecast how much a stock price changes and another to predict what direction the stock price will change (up or down). \\


Besides, as this system is build up on cluster computing framework, it is also important to find out the best data mining methods over distributed environment
